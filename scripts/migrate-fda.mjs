...
await client.query('ALTER TABLE regimens ADD COLUMN IF NOT EXISTS fda_approved BOOLEAN DEFAULT FALSE;');
...
if (fs.existsSync(jsonPath)) {
  ...
  for (const name of approvedNames) {
    const r = await client.query('UPDATE regimens SET fda_approved = TRUE WHERE LOWER(drug) = $1', [name]);
    count += r.rowCount || 0;
  }
